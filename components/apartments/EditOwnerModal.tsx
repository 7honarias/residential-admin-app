...
import { Owner, OwnerForm } from "@/app/dashboard/apartments/apartment.types";
...
interface EditOwnerModalProps {
  ...
  onSave: (owner: Owner) => Promise<void>;
}
...
export default function EditOwnerModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      await onSave(form as OwnerForm);
      onClose();
    } catch (error) {
      ...
      <div className="relative bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 animate-in fade-in zoom-in-95">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-lg font-semibold text-slate-800">
            {currentOwner ? "Modificar propietario" : "Asignar propietario"}
          </h2>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="text-sm font-medium text-slate-600">Confirmar Email</label>
            <input
              type="email"
        ...
        <div className="flex justify-end gap-3 mt-8">
          ...
          <button
            ...
            disabled={loading || (!!form.confirmEmail && form.email !== form.confirmEmail)}
